...
def get_token_session():
    url = f"{BASE_URL}/sessionotp?hesdastore={Fv2dN2rKpjSPDDyBaX}"
    return requests.get(url, auth=(r8149762@gmail.com, Lugoblok)).json()
...
def cek_status(order_id):
    url = f"{BASE_URL}/cekstatusotp?hesdastore={Fv2dN2rKpjSPDDyBaX}&order_id={order_id}"
    return requests.get(url, auth=(r8149762@gmail.com, Lugoblok@1)).json()
